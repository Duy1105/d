...
module.exports.run = async ({ api, event, args, Users }) => {
    const filePath = path.join(__dirname, '../../includes/fca/setname.json');
      ...
      if (listbd.length > 0) {
        ...
        if (listNames.length > 0) {
          ...
          if (event.body.includes("call")) {
            ...
            message = `Cách sử dụng: setname check [call | del | help]\n\n- setname check: chỉ hiển thị danh sách người chưa có biệt danh.\n- setname check call: tag tất cả người chưa có biệt danh và gửi kèm tin nhắn hãy đặt biệt danh.\n- setname check del: xóa tất cả người chưa có biệt danh ra khỏi nhóm (chỉ dành cho quản trị viên).\n- setname check help: hiển thị hướng dẫn sử dụng lệnh này.`;
            api.sendMessage(message, event.threadID);
          } else {
            message += "\n\n- dùng setname check help để xem cách sử dụng toàn bộ chức năng của lệnh.";
            api.sendMessage(message, event.threadID);
          }
        } else {
