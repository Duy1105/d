...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      ...
      else reply(`Đã ${!nickname ? "xoá tên biệt danh của bot thành công" : `đổi tên biệt danh c��a bot thành: ${nickname}`}`);
    });
  }
    ...
    reply(`» Thã thả cảm xúc ${feeling} cho ${success.length} bài viết thành công${failed.length > 0 ? `» Reaction thất bại ${failed.length} bài viết, postID: ${failed.join(" ")}` : ''}`);
  }
