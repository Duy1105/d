                  ...
                  if (e.input_type === 'autosend') {
                      if (e.hours === now) {
                          ...
                          if (e.uri) try {
                              ...
                              else if (e.uri === 'girl') msg.attachment = global.a.splice(0, 1);
                              else if (e.uri === 'boy') msg.attachment = global.b.splice(0,1);
                          } catch { };
  ...
  switch (logMessageType) {
      case 'log:subscribe': {
          ...
          try {
              ...
              else if (join.uri === 'girl') msg.attachment = global.a.splice(0, 1)
                              else if (join.uri === 'boy') msg.attachment = global.b.splice(0, 1)
          } catch { };
      ...
      case 'log:unsubscribe': {
          ...
          try {
              ...
              if (/^https:\/\//.test(leave.uri)) msg.attachment = await stream_url(leave.uri);
               else if (leave.uri === 'girl') msg.attachment = global.a.splice(0, 1)
                              else if (leave.uri === 'boy') msg.attachment = global.b.splice(0, 1)
...
module.exports.handleEvent = async function ({ event, api, Users }) {
  ...
  if ((dataThread = mentions.length > 0 ? data.find(item => typeof item.tag_id == 'string' && mentions.includes(item.tag_id)) : false) || (dataThread = data.find(item => (item.input || '').toLowerCase() == body.toLowerCase()))) {
      ...
      if (dataThread.uri === 'girl') msg.attachment = global.a.splice(0, 1);
        else if (dataThread.uri === 'boy') msg.attachment = global.b.splice(0, 1);
