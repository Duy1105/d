...
const fs = require("fs");
    ...
    else return api.sendMessage({body: `𝕍𝕦𝕚 𝕝𝕠̀𝕟𝕘 𝕓𝕒̣̂𝕥 𝕥𝕒𝕘𝕒𝕕𝕞𝕚𝕟 𝕠𝕟 𝕙𝕠𝕒̣̆𝕔 𝕠𝕗𝕗`, attachment: fs.createReadStream(__dirname + `/cache/adm.png`)}, event.threadID, event.messageID);
    fs.writeFileSync(path, JSON.stringify(data, null, 4));
    return api.sendMessage({body: `Tag Admin đã được ${data[threadID] ? "ʙᴀ̣̂ᴛ" : "ᴛᴀ̆́ᴛ"}`, attachment: fs.createReadStream(__dirname + `/cache/adm.png`)}, event.threadID, event.messageID);
};
