...
var coinscn = Math.floor(Math.random() * 1000) + 1000; //random coins khi làm ở khu công nghiệp
var coinsdv = Math.floor(Math.random() * 800) + 1000; //random coins khi làm ở khu dịch vụ
var coinsmd = Math.floor(Math.random() * 700) + 1000; //random coins khi làm ở mỏ dầu
var coinsq = Math.floor(Math.random() * 600) + 1000; //random coins khi khai thác quặng
var coinsdd = Math.floor(Math.random() * 500) + 1000; //random coins khi đào đá
var tdh = Math.floor(Math.random() * 800) + 2000; //random coins khi làm cave ở Trần Duy Hưng
var pnl = Math.floor(Math.random() * 801) + 5000; //random coins khi làm cave ở Phạm Ngũ Lão
var nht = Math.floor(Math.random() * 401) + 2000; //random coins khi làm cave ở Nguyễn Huy Tự
//random ku dài
...
var msg = "";
    switch(handleReply.type) {
        case "choosee": {
            ...
            switch(event.body) {
                case "1": msg = `⚡️𝑆𝑎𝑢 1 đ𝑒̂𝑚 𝑏𝑎̣𝑛 đ𝑢̛𝑜̛̣𝑐 ${work5} 𝑐𝑢 𝑑𝑎̀𝑖   ${ku2} ở 𝐓𝐫𝐚̂̀𝐧 𝐃𝐮𝐲 𝐇𝐮̛𝐧𝐠 cho số tiền là  ${pnl}vnd`; Currencies.increaseMoney(event.senderID, pnl); break;            
