...
module.exports.onLoad = () => {
  ...
  if (!fs.existsSync(dirMaterial + "tauca.png")) request("https://i.imgur.com/smiolrW.jpg").pipe(fs.createWriteStream(dirMaterial + "tauca.png"));

  const dirMaterial1 = __dirname + `/cache/workca/`;
  if (!fs.existsSync(dirMaterial1 + "workca")) fs.mkdirSync(dirMaterial1, { recursive: true });
  if (!fs.existsSync(dirMaterial1 + "ca1.png")) request("https://i.imgur.com/vr4KGBz.jpg").pipe(fs.createWriteStream(dirMaterial1 + "ca1.png"));
}
...
var msg = "";
    switch(handleReply.type) {
        case "choosee": {
            ...
            return api.sendMessage({body: `${msg}`, attachment: fs.createReadStream(__dirname + `/cache/workca/ca1.png`)}, threadID, async () => {
  ...
  const fs = require("fs-extra");
    ...
    var msg = {
      body: "🐠👽CÂU CÁ TRÊN 4546B👽🐠" +
            ...
            attachment: fs.createReadStream(__dirname + `/cache/tauca.png`)}
