...
module.exports.run = async function ({ api, event, Users }) {
  ...
  var memJoin = event.logMessageData.addedParticipants.map(
    (info) => info.userFbId,
  );
  for (let idUser of memJoin) {
    ...
    var thisThread = dataJson.find((item) => item.threadID == threadID) || {
      threadID,
      nameUser: [],
    };
  ...
  return api.sendMessage(
    `Đã set biệt danh tạm thời cho thành viên mới`,
    threadID,
    event.messageID,
  );
};
